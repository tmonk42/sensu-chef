  ...
  describe ".gem_binary" do
    context 'on unix-like platforms' do
      context 'with omnibus ruby available' do
        before do
          allow(File).to receive(:exist?).with(unix_omnibus_gem_path).and_return(true)
          allow(File).to receive(:exist?).with(windows_omnibus_gem_path).and_return(false)
        end
      ...
      context 'without omnibus ruby available' do
        before do
          allow(File).to receive(:exist?).with(unix_omnibus_gem_path).and_return(false)
          allow(File).to receive(:exist?).with(windows_omnibus_gem_path).and_return(false)
        end
    ...
    context 'on windows platforms' do
      ...
      context 'with omnibus ruby available' do
        before do
          allow(File).to receive(:exist?).with(unix_omnibus_gem_path).and_return(false)
          allow(File).to receive(:exist?).with(windows_omnibus_gem_path).and_return(true)
        end
